...
Tend        = 2
K_range     = c(0.5, 2)
b_range     = c(0.6, 0.99) # Set to c(0, 0) if not modelling a barrier option (0.6, 0.99)
eta_range   = c(0, 0.3) # Set to c(0, 0) if not modelling a barrier option (0, 0.3)
...
param_num   = 124 # Number of random parameter settings to be used for each date and maturity combination
...
training_cols           = c("S_K", "T_t", "r", "sigma", "b", "eta") # Vector of columns to be used in training the model(s) (set up for "S_K", "T_t", "b", "eta", "r", "sigma", "dummy")
...
params                  = list(list(epochs = 10, batch_size = 64, units = c(200, 100), L2_factor = 0.0001,
                                    activation = "relu", initializer = "glorot_uniform", dropout = 0,
                                    optimizer = "adamax", momentum = 0))
